...
train = df_stock_info.loc[maxlength-(480+sequence_length):maxlength-121, ['close', 'volume', 'diff']]
test = df_stock_info.loc[maxlength-(120+sequence_length):, ['close', 'volume', 'diff']]
...
train_sc_df = pd.DataFrame(train_sc, columns=['종가', '거래량', '전일비'], index=train.index)
test_sc_df = pd.DataFrame(test_sc, columns=['종가', '거래량', '전일비'], index=test.index)
...
X_train = train_sc_df.dropna().drop(['종가', '거래량', '전일비'], axis=1)
...
X_test = test_sc_df.dropna().drop(['종가', '거래량', '전일비'], axis=1)
...
X_train_t = X_train.reshape(X_train.shape[0], sequence_length*3, 1)
X_test_t = X_test.reshape(X_test.shape[0], sequence_length*3, 1)
...
model.add(LSTM(50, input_shape=(sequence_length*3, 1)))# (timestep, feature)
# model.add(Dropout(0.5))
model.add(Dense(1)) # output = 1
model.compile(loss='mean_squared_error', optimizer='adam')

# loss를 모니터링해서 patience만큼 연속으로 loss률이 떨어지지 않으면 훈련을 멈춘다.
early_stop = [EarlyStopping(monitor='val_loss', patience=50, verbose=1), ModelCheckpoint(filepath='best_model_diff', monitor='val_loss', save_best_only=True)]

# history=model.fit(X_train_t, Y_train, epochs=100, batch_size=30, verbose=1, callbacks=[early_stop])

history = model.fit(X_train_t, Y_train, epochs=100000, verbose=2, batch_size=120, validation_data=(X_test_t, Y_test), callbacks=early_stop)
